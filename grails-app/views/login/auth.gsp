            ...
            <form class="form-signin" action="${postUrl ?: '/login/authenticate'}" method="POST" id="loginForm" autocomplete="off">
              ...
              <div class="form-group form-check">
                  <label class="form-check-label">
                      <input type="checkbox" class="form-check-input" name="${rememberMeParameter ?: 'remember-me'}" id="remember_me" <g:if test='${hasCookie}'>checked="checked"</g:if>/> Lembrar
                </label>
              </div>
              <button id="submit" class="btn btn-lg btn-primary btn-block text-uppercase" type="submit">Entrar</button>
              <hr class="my-4">
              <p>Ainda não possui conta? <g:link controller="usuario" action="cadastro">Cadastro</g:link></p>
               <p>Esqueceu a senha ? <g:link controller="usuario" action="recoveryPassword">Recuperar</g:link></p>
            </form>
          </div>
        </div>
      </div>
